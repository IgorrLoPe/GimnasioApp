...
import storage.UsuarioRepository
...
fun main() {
    ...
    //cargo lo que haya en el json si este esta creado
  ...
  when (readLine()?.trim()) {
    "1" ->{
     ...
     var idingresada = readLine()!!
      println("ingresa el nombre de usuario (minimo una mayuscula): ")
            ...
            try {
                ...
                if (miRegex.validarusuario(nombreingresado)){
                    ...
                    if (Gimnasio.agregarusuario(idingresada, nombreingresado)) {
                        ...
                        usuarioiniciado = 1
                        //guardo el usuario personaActual con !! por que tiene que haver un valor
                        repoUsuarios.save(PersonaActual!!)
                    }  }else{
      ...
      "2"->{
          ...
          try {
              ...
              val user = repoUsuarios.findById(idindicado)

              if (user != null && user.nombre == nombreindicado) {
